...
import Logo from "../../assets/images/Logo.png"; // ✅ Logo import added
import ApiService from "../../ApiServices/ApiServices";
import ChatDropdown from "../chatdropdown/chat";
    ...
    const fetchProfileDetails = async () => {
      try {
        ...
        try {
          ...
          const baseUrl = process.env.REACT_APP_API_BASE_URL || "API_URL2";
          const fullImageUrl = `${baseUrl.replace(/\/$/, "")}${url}`;
          setProfileImage(fullImageUrl);
          localStorage.setItem("profileImage", fullImageUrl);
        }
...
useEffect(() => {
  const fetchUnreadChats = async () => {
    try {
      ...
      setUnreadChatCount(res.total_unread_count || 0);
    } catch (err) {
  ...
  fetchUnreadChats();
  const interval = setInterval(fetchUnreadChats, 30000); // refresh every 30s
  return () => clearInterval(interval);
}, []);
...
{["auditor", "viewer", "reconciliation admin"].includes(role.toLowerCase()) && (
  <div className="chart-icon" style={{ position: "relative" }}>
    <MessageOutlinedIcon
    ...
    {activeDropdown === "chat" && (
      <ChatDropdown onClose={() => setActiveDropdown(null)} />
    )}
  </div>
)}
