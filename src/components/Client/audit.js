...
const [selectedAuditorFindings, setSelectedAuditorFindings] = useState([]);
  ...
  const [auditorRemarkModalOpen, setAuditorRemarkModalOpen] = useState(false);
const [selectedAuditorRemark, setSelectedAuditorRemark] = useState("");
...
<TableCell sx={{ fontSize: "12px", textAlign: "center" }}>
  ...
  {row.auditorRemark !== "-" ? (
    <Button
      ...
      size="small"
      onClick={() => {
        setSelectedAuditorRemark(row.auditorRemark);
        setAuditorRemarkModalOpen(true);
      }}
      sx={{
        fontSize: "11px",
        textTransform: "none",
...
>
  ...
  <DialogActions sx={{ padding: 2 }}>
    ...
    >
      ...
      {/* Auditor Remark Modal */}
<Dialog
  open={auditorRemarkModalOpen}
  onClose={() => setAuditorRemarkModalOpen(false)}
  maxWidth="sm"
  fullWidth
  PaperProps={{
    sx: { borderRadius: 2, padding: 1 }
...
>
  ...
  <DialogContent dividers>
    <Typography variant="body1" sx={{ whiteSpace: "pre-wrap" }}>
      {selectedAuditorRemark}
    </Typography>
  </DialogContent>
  <DialogActions sx={{ padding: 2 }}>
    <Button
      onClick={() => setAuditorRemarkModalOpen(false)}
      variant="contained"
      color="primary"
      sx={{ textTransform: "none" }}
    >
