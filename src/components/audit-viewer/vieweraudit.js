...
useEffect(() => {
  const fetchUserDetails = async () => {
    try {
      const res = await apiServices.details();

      // 🔹 Extract organization
      const org = res.details?.find((item) => item.organization)?.organization;
      if (org?.company_name) {
        setusername(org.company_name);   // ✅ only org company_name
        console.log("✅ Company Name:", org.company_name);
      }
...
useEffect(() => {
  const fetchOrgId = async () => {
    try {
      const res = await apiServices.details(); // calling details/ API
      // Navigate to the organization id
      const org = res.details?.find((item) => item.organization)?.organization;
      if (org?.id) {
        setOrgId(org.id);
        console.log("✅ Org ID loaded:", org.id);
      } else {
        console.error("❌ Organization ID not found in details response");
      }
    } catch (err) {
...
// Listen for status change events (instant update from chatbox, etc.)
useEffect(() => {
  const handleStatusChange = (event) => {
    ...
    setTableData((prevData) =>
      prevData.map((item) =>
        item.id === declarationId ? { ...item, status: newStatus } : item
      )
    );
  };
...
useEffect(() => {
  ...
  const pollStatusUpdates = async () => {
    try {
      ...
      setTableData((prevData) =>
        prevData.map((item) => {
          // Find matching declaration by ID or declaration number
          const freshItem = response.find((r) => 
            r.id === item.id || 
            r.declaration_number === item.declarationNum
          );
          
          if (freshItem) {
            // Map backend status to frontend status
            let newStatus = "Open";
            const backendStatus = freshItem.workflow_status || freshItem.status;
            
            if (backendStatus === "Pending With Client" || backendStatus === "Pending with Client") {
              newStatus = "Pending with Client";
            } else if (backendStatus === "Pending With Auditor" || backendStatus === "Pending with Auditor") {
              newStatus = "Pending with Auditor";
            } else if (backendStatus === "Closed") {
              newStatus = "Closed";
            } else if (backendStatus === "Open") {
              newStatus = "Open";
            }

            // Only update if status changed
            if (newStatus !== item.status) {
              console.log("🔄 Status updated:", item.declarationNum, "from", item.status, "to", newStatus);
...
useEffect(() => {
  ...
  const filteredData = tableData.filter(row => {
    ...
    // Check all relevant fields including declarationDate
    const matchesSearch =
      (row.declarationNum?.toLowerCase() || '').includes(searchLower) ||
      (row.declarationDate?.toLowerCase() || '').includes(searchLower) || // Added declarationDate
...
const handleOpenSpecificDocument = async (doc) => {
  ...
  try {
    ...
    // ✅ After opening, update the status in tableData
    setTableData((prevData) =>
      prevData.map((row) => {
        if (row.declarationNum === doc.declaration_number && row.status === "Open") {
          return { ...row, status: "Pending with Auditor" };
        }
        return row;
      })
    );
...
const handleSendMessage = async (docId, message, mode) => {
  ...
  // ✅ ADD THIS: Update status instantly if Closed (like AuditorDeclaration)
  if (currentDocument && currentDocument.status === "Closed") {
    try {
      const token = localStorage.getItem("token");
      const newStatus = "Pending with Auditor"; // Viewer sends message -> Pending with Auditor
      ...
      setTableData(prevData =>
        prevData.map(row =>
          row.id === currentDocument.id ? { ...row, status: newStatus } : row
        )
      );
...
<TableCell sx={{ fontSize: "12px" }}>
  <TextField
    ...
    onChange={(e) => handleClientResponseChange(e, row.id)}
    disabled={row.status === "Closed" || (row.auditorFinding === "-" && row.auditorRemark === "-")}
    sx={{
      minWidth: 90,
      maxWidth: "100%",
      fontSize: '12px',
...
<TableCell sx={{ fontSize: "12px" }}>
  <TextField
    ...
    value={row.clientRemarks || ""}
    onClick={() => handleOpenRemarkModal(row.id, row.clientRemarks)}
    disabled={row.status === "Closed" || (row.auditorFinding === "-" && row.auditorRemark === "-")}  
    InputProps={{
      readOnly: true,
      endAdornment: row.clientResponse ? (
        <InputAdornment position="end">
          <IconButton
            size="small"
            onClick={(e) => {
              e.stopPropagation();
              handleOpenRemarkModal(row.id, row.clientRemarks);
            }}
            sx={{ p: 0 }}
          >
    ...
    sx={{
      '& .MuiOutlinedInput-root': {
        paddingRight: row.clientResponse ? '8px' : '14px',
      },
      '& .MuiInputBase-input': {
                      ...
                      <Tooltip title="ChatBox">
                        <IconButton
                          ...
                          onClick={() => handleOpenChat(row)}
                           disabled={row.auditorFinding === "-" && row.auditorRemark === "-"}
                        >
<Badge
  color="error"
  variant="dot"
...
<ChatBox
  ...
  currentDocument={{
    ...
    thread_type: 'Audit',
    declarationNum: currentDocument?.declarationNum || 'Unknown',
    status: localStatus // Pass the local status
  }}
