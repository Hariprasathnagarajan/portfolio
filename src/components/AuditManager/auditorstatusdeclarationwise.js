...
const headCells = [
  ...
  { id: "actionType", label: "Action", align: "center", width: "10%" },
...
const { organization, username, statusFilter: initialStatusFilter, auditorName } = location.state || {};
  ...
  const filteredData = data.filter(row => {
    const matchesSearch =
      row.declarationNum.toLowerCase().includes(searchTerm.toLowerCase()) ||
      row.declarationDate.toLowerCase().includes(searchTerm.toLowerCase()) ||
      row.auditorFinding.toLowerCase().includes(searchTerm.toLowerCase()) ||
      row.auditorRemark.toLowerCase().includes(searchTerm.toLowerCase()) ||
      row.clientResponse.toLowerCase().includes(searchTerm.toLowerCase()) ||
      row.clientRemarks.toLowerCase().includes(searchTerm.toLowerCase());

    const matchesFilter =
      (monthFilter === "All" || row.declarationDate === monthFilter) &&
          ...
          <Box sx={{ display: "flex", gap: 3 }}>
            ...
            <Typography
                      ...
                      >
                        ...
                        {name || "N/A"} {/* Display auditor name here */}
                      </Typography>
          </Box>
        </Box>
      </Box>
