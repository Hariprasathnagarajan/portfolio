...
const headLabel = [
  ...
  { id: 'type', label: 'Type', align: 'left' }, // New Type field
...
const [auditorUUID, setAuditorUUID] = useState('');
const [auditorName, setAuditorName] = useState('');

console.log("🟠 Auditor UUID from location:", auditorUUID);

useEffect(() => {
  const fetchAuditorAndOrganizations = async () => {
    try {
      const details_data = await apiServices.details();
      const user = details_data.details[3]; // Custom user
      const role = details_data.details[5]; // Role object

      if (role.name === 'Auditor') {
        const auditorUUID = user.id;
        const auditorName = user.auth_user.first_name;

        console.log("✅ auditorUUID being sent:", auditorUUID);
        console.log("✅ auditorName from details:", auditorName);

        setAuditorUUID(auditorUUID);
        setAuditorName(auditorName);

        const response = await apiServices.getAuditorOrganizationStatus(auditorUUID);
        setData(response);
      } else {
        console.warn("User is not an auditor.");
      }
    } catch (err) {
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter(org => {
      return !searchTerm ||
        ...
        org.type.toLowerCase().includes(formattedSearchTerm); // Include type in search
    });
  }, [data, searchTerm]);
  ...
  const sortedData = useMemo(() => {
    return [...filteredData].sort((a, b) => {
      ...
      else if (orderBy === 'type') comparison = a.type.localeCompare(b.type); // Sort by type
...
const handleCountClick = (type, count, orgId, orgName, username) => {
  if (count === 0) return;
  ...
  navigate("/auditor-declaration", {
    state: {
      orgId: orgId,                // ✅ Now using organization ID
              ...
              {visibleRows.map(row => (
                ...
                >
                  <TableCell align="left">
                    <Link
                      to="/auditor-declaration"
                      state={{ orgId: row.id, organization: row.organization, username: row.username, auditorName: auditorName }} // Pass the required data
                  ...
                  <TableCell align="left">
                    <Tooltip title={row.type} arrow>
                      <span>{row.type}</span> {/* Display the Type field */}
                    </Tooltip>
                  </TableCell>
...
<TableCell align="center">
  <Tooltip title="View open items" arrow>
    <Box
      onClick={() => handleCountClick('open', row.open, row.id, row.organization, row.username)}
      sx={{
...
<TableCell align="center">
  <Tooltip title="View Pending with client items" arrow>
    <Box
      onClick={() => handleCountClick('pendingClient', row.pendingClient, row.id, row.organization, row.username)}
      sx={{
...
<TableCell align="center">
  <Tooltip title="View Pending with auditor items" arrow>
    <Box
      onClick={() => handleCountClick('pendingAuditor', row.pendingAuditor, row.id, row.organization, row.username)}
      sx={{
...
export default AuditorCompanyStatus;
