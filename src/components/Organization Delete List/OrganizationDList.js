...
const OrganizationList = () => {
    ...
    const handleSelectAll = (e) => {
        if (e.target.checked) {
            const allIds = paginatedData.map(org => org.id);
  ...
  <div className="organization-actions">
    <button
      ...
      onClick={async () => {
        if (window.confirm("Are you sure you want to permanently delete selected organizations?")) {
          setIsLoading(true);
          try {
    ...
    <button
      ...
      onClick={async () => {
        ...
        try {
          for (const id of selectedIds) {
            await apiServices.restoreOrganization(id); // Backend API
          }
          fetchOrganization();
          setSelectedIds([]);
        } catch (error) {
                    ...
                    {paginatedData.length > 0 ? (
                        paginatedData.map((org, index) => (
                            ...
                            <tr key={index} className="organization-table-row">
                                ...
                                <td className="organization-table-td">
                                    {org.msa_doc ? (
                                        <a href={url + org.msa_doc} title={org.msa_doc.split('/').pop()} target='_blank' rel='noopener noreferrer'>
                                            {org.msa_doc.split('/').pop().substring(0, 30) + '...'}
                                        </a>
                                    ) : "Null"}
                                </td>
                                <td className="organization-table-td">
                                    {new Date(org.created_date).toLocaleDateString('en-US', {
                                        year: 'numeric', month: 'long', day: 'numeric'
                                    })}
                                </td>
                                <td className="organization-table-td">
                                    <button className="organization-delete" onClick={() => handleDelete(org.id)}>
